...
const form = reactive({
    nama_tempat: props.catatan.nama_tempat,
    alamat: props.catatan.alamat,
    tanggal: props.catatan.tanggal,
    jam: props.catatan.jam,
    suhu_tubuh: props.catatan.suhu_tubuh,
    kendaraan: props.catatan.kendaraan,
});

const updateCatatan = (id) => {
    Inertia.put(`/catatan/${id}`, form)
};
</script>
...
<template>
    ...
    <template #main>
            <section class="max-w-4xl p-6 my-5 mx-auto text-white bg-gray-800 rounded-xl shadow-xl">
                <h2 class="text-2xl capitalize dark:text-white">Edit Catatan</h2>
                ...
                <form @submit.prevent="updateCatatan(`${catatan.id}`)">
                    <div class="grid grid-cols-1 gap-6 mt-4 text-white">
                        <div>
                            <label for="nama_tempat">Nama Tempat</label>
                            <input id="nama_tempat" type="text" v-model="form.nama_tempat" placeholder="Masukkan nama_tempat" class="block w-full px-4 py-2 mt-2 bg-white border border-gray-900 text-gray-800 rounded-lg focus:ring-main focus:ring-opacity-40 focus:border-main focus:outline-none focus:ring" required>
